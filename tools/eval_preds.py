...
from tools.cocoeval_adjusted import COCOeval
import openpyxl
from tools.filter_gts import filter_eval

path = os.getcwd()[:-6]


def read_json(filename="".join([path, '\\annotations\\annotations_extended_coco_kpt.json'])):
...
def write_excel(model_name, input_size, data_subset, data_variable, ms, results):
    try:
        wb = openpyxl.load_workbook("".join([path, "\\results\\results_new_labels.xlsx"]))
    except:
    ...
    if check:
        wb.save("".join([path, "\\results\\results_new_labels.xlsx"]))


def run_tests_models(model_name, input_size, ms, model_results):
    filter_eval(0, 2, coco_val2017, annotations)

    res = evaluate_results(kps_data=COCO("".join([path, '\\annotations\\filtered_by_challenges_coco_val2017.json'])),
...
coco_val2017 = "".join([path, '\\coco\\data\\person_keypoints_val2017.json'])


def test_all_models(test_function):
    # import these models from github
    # https://github.com/leoxiaobin/deep-high-resolution-net.pytorch
    ResNet_152_384x288 = read_json("".join([path, "\\models\\ResNet_152_384x288.json"]))  # models//ResNet_152_384x288.json
    HRNet_w48_384x288 = read_json("".join([path, "\\models\\HRNet_w48_384x288.json"]))

    # https://github.com/HRNet/DEKR
    DEKR_w48_640x640_with_multi_scale = read_json("".join([path, "\\models\\DEKR_w48_640x640_with_mutli_scale.json"]))

    # https://github.com/ilovepose/DarkPose
    DarkPose_w48_384x288 = read_json("".join([path, "\\models\\DarkPose_w48_384x288.json"]))
